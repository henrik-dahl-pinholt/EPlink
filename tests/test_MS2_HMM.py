...
def test_cp_state():
    ...
    assert np.allclose(
        np.mean(a_states_cp[200:], axis=0), [1 - p_on_theory, p_on_theory], atol=0.01
    )
    assert np.allclose(
        np.mean(a_states_cp[200:], axis=0), [1 - p_on_theory, p_on_theory], atol=0.01
    )
...
def test_Generate_trajectory():
    ...
    samples = MS2_HMM.Generate_sample(
        k_on_driver, k_off, state_0, nsamples, nstates, dt, verbose=False
    )

    theory_line = np.exp(-(k_off + k_on) * np.arange(nsteps + 1) * dt) + (
        k_on / (k_off + k_on)
    ) * (1 - np.exp(-(k_off + k_on) * np.arange(nsteps + 1) * dt))
...
def test_mean_calc():
    ...
    window_size = 10
    tau = -0.1
    loading_rates = np.array([1.5, 4.0, 10.0])

    smap, state_sequences = MS2_HMM.Generate_state_map(nstates, window_size)

    state_emission_means = MS2_HMM.swap_inds_for_rates(
        state_sequences, loading_rates
    ) @ MS2_HMM.MS2_kernel(np.arange(window_size), tau, window_size)
    assert state_emission_means[0] == 15.0
    assert state_emission_means[1] == 17.5
    assert state_emission_means[-1] == 100.0

    # check that it works when it takes a finite time to get through
    tau = 2.0
    loading_rates = np.array([1.0, 4.0, 10.0])
    ...
    assert state_emission_means[-1] == 85.0
